...
RSpec.describe TasksController, type: :controller do
  describe "tasks#index" do
    it "should list the tasks in the database" do
      ...
      response_ids = []
      response_ids = response_value.collect do |task|     #collect transforms array and builds new response_ids =  << task["id"] 
        task["id"]
      end
  ...
  describe "task#create" do
    it "should allow new tasks to be created" do
      post :create, task: {title: "Firehose Project"}
      expect(response).to have_http_status(:success)
      response_value = ActiveSupport::JSON.decode(@response.body)
      expect(response_value['title']).to eq("Firehose Project")
      expect(Task.last.title).to eq("Firehose Project")
    end
  end
end
